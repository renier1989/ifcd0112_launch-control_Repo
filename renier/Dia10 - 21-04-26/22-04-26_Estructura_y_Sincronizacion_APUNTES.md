...
graph TD
    ...
    root --> sync_curso["📁 sync_curso/"]
    sync_curso --> mi_proyecto["📂 mi_proyecto/ <br/><i>(Espejo carpeta compartida Windows)</i>"]
    
    %% Rama de Launch Control
    root --> lc["📁 launch-control/"]
    
    lc --> lc_conf["📁 config/"]
    lc_conf --> db_conf["📄 database.conf <br/><i>(DB Config)</i>"]
    lc_conf --> net_conf["📄 network.conf <br/><i>(Red Config)</i>"]
    
    lc --> lc_scripts["📁 scripts/"]
    lc_scripts --> health["🐚 health_check.sh <br/><i>(Verificación)</i>"]
    
    lc --> lc_logs["📁 logs/ <br/><i>(Registros)</i>"]
    lc --> lc_data["📁 data/ <br/><i>(Datos App)</i>"]
    lc --> lc_docs["📁 docs/ <br/><i>(Documentación)</i>"]
    lc --> lc_env["🔑 .env <br/><i>(Variables sensibles - 600)</i>"]
    
    %% Archivos de Raíz
    root --> sinc_sh["📜 sincronizar.sh <br/><i>(Script de sincronización)</i>"]
    root --> sinc_log["📝 .sync_curso.log <br/><i>(Log del script)</i>"]

    %% Estilos para mejorar la visualización
    style root fill:#f5f5f5,stroke:#333,stroke-width:2px
    style lc_env fill:#fff4dd,stroke:#d4a017
    style mi_proyecto fill:#e1f5fe,stroke:#01579b
    style sinc_sh fill:#e8f5e9,stroke:#2e7d32
```
...
### Verificacion del archivo .sh con el shellcheck
```bash
sudo apt update
sudo apt install shellcheck
shellcheck ~/launch-control/scripts/health_check.sh
```
...
graph TD
    subgraph Host ["💻 WINDOWS (Host)"]
        W_Dir["📂 Carpeta del Proyecto <br/> (Cualquier ubicación)"]
    end

    subgraph VBox ["⚙️ VirtualBox Hypervisor"]
        VB_Config["Configuración ➔ Carpetas compartidas <br/> (Auto-montaje habilitado)"]
    end

    subgraph VM ["🐧 LINUX (VM)"]
        SF_Dir["🔒 /media/sf_mi_proyecto/ <br/> (root:vboxsf)"]
        Work_Dir["🛠️ ~/sync_curso/mi_proyecto/ <br/> (TU usuario)"]
        
        SF_Dir -- "🔄 rsync -a --delete" --> Work_Dir
    end

    %% Flujo entre entornos
    W_Dir --> VB_Config
    VB_Config --> SF_Dir

    %% Estilos y Notas
    style SF_Dir fill:#ffebee,stroke:#c62828,stroke-width:2px
    style Work_Dir fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
    
    classDef note font-size:12px,fill:#fff,stroke-dasharray: 5 5;
    
    SF_Note["⚠️ NO escribir aquí <br/> (Problemas de permisos)"]:::note
    Work_Note["✅ ZONA DE TRABAJO <br/> (Permisos correctos)"]:::note
...
sino despuesta de hacer lo indicado, reinciar la maquina virtual y volver a verificar que el vboxsf exista dentro de los groups
