...
|---|---|
| `/home` |es donde se guardan los archivos de los usuarios del sistema | 
| `/etc` |es donde se guardar los archivos de configuracion | 
| `/var/log` |es donde se guardan los logs del sistema | 
| `/tmp` |es donde se registran de forma temporal archivos del sistema | 
| `/bin` | es donde se guardar archivos ejecutables del sistema| 
| `/root` |es la carpeta del usuario root
 | 
### A.2 Verdadero o Falso

Marquen V (verdadero) o F (falso) y corrijan las falsas:

1. En Linux, `Archivo.txt` y `archivo.txt` son el mismo archivo. [F ] 
> Son archivos diferentes porque linux es Keysensitive y no distingue entre mayusculos y minusculas
2. El comando `rm` envía los archivos a la papelera de reciclaje. [F ]
> en Linux no existe la papelera de reciclaje si se trata de la linea de comandos
3. El operador `>>` sobreescribe el contenido de un archivo. [V ]
> pero solo agrega lineas o contenido al final del archivos si es que ya existe algo previamente
4. El símbolo `~` equivale al directorio `/home/alumno`. [V ]
5. El permiso `755` significa que el propietario puede leer, escribir
   y ejecutar, y los demás solo leer y ejecutar. [V ]
6. `sudo` permite ejecutar un comando como administrador. [V ]
7. Los archivos ocultos en Linux empiezan por `_` (guion bajo). [F ]
> Los archivos ocultos en Linux empiezan por `.` (punto)
8. El comando `mkdir -p a/b/c` falla si el directorio `a` no existe. [F ]
> El comando `mkdir -p a/b/c` crea el directorio `a` si no existe, y luego crea el directorio `b` dentro de `a`, y finalmente crea el directorio `c` dentro de `b`.
9. En el prompt `alumno@ubuntu2204:~$`, el `$` indica que son root. [F ]
> $ indica que es un usuario normal, el # indica que es el usurio root
10. El comando `chmod 644 archivo.txt` da permiso de ejecución al propietario. [F ]
> El comando `chmod 644 archivo.txt` da permiso de lectura y escritura al propietario, y permiso de lectura a los demás.
...
> Respuesta : ![alt text](image-2.png)
...
2. Cambien los permisos de `saturn_v.txt` a:
   ...
   - Grupo: solo leer
   - Otros: nada
   (¿Qué número octal es?)´ : > 640

   > Repuesta: ![alt text](image-8.png)
3. Cambien los permisos de `missions/manifest.txt` a 755.
   ¿Qué significa eso en la práctica? ¿Tiene sentido para un .txt?
> Respuesta:  significa que el propietario puede leer, escribir y ejecutar, el grupo solo puede leer y ejecutar, y los demas solo pueden leer y ejecutar
...
>Respuesta : ![alt text](image-11.png)
...
> Respuesta : ![alt text](image-12.png)
...
> Respuesta: Ejecucion del comando para la creacion de las carpetas 
![alt text](image-13.png)
````bash 
mkdir -p artemis-iii/{crew/{commander,pilot,specialist},vehicle/{sls,orion,hls},mission_control/{procedures,comms},science/{experiments,samples},logs,scripts}
```` 
...
comprobacion de la estructura de artemis-iii: 
...
> Resultado de la ejecucion del preflight_check.sh
![alt text](image-29.png)

> Resultado de la ejecucion del telemetry.sh
...
```
> creacion de script para cracion de status_report.txt
![alt text](image-18.png)

> Resultado de la ejecucion del script y creacion del archivo: 
...
> resultado de la ejecucion del sh de backup.sh
...
|---|---|
| Del 1 al 5, ¿cómo de cómodo me siento con la terminal? |5 |
| ¿Qué comando me cuesta más recordar? |filtros y busquedas |
| ¿He necesitado mirar el manual para el diagnóstico? |Si |
| ¿He podido ayudar a algún compañero esta semana? |Si |
| ¿Qué es lo que más me ha sorprendido de Linux? |Potencia Velocidad |
| ¿Qué es lo que más me ha costado? |Scripts automatizacion |
